...
func Logrecord(title, msg string) (string, error) {
...
func ActionDetailrecord(title, msg string) (string, error) {

	fileName := fmt.Sprintf("%s/details_%s.log", global.EnvConfig.Log.Path, time.Now().Format("200601"))
	// open file and create if non-existent
	file, err := os.OpenFile(fileName, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		return "", fmt.Errorf("failed to open detail log file %s: %w", fileName, err)
	}
	defer file.Close()
